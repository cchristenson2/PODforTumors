function [N_aug,kp] = Augment_wMC_wTRX_2D(N, t, h, dt, bcs, bounds, ntp_cal, tx_params, M, E, nu, matX, matY)
    thresh = 0.15;
    ...
    %% Run forward eval
        if(ntp_cal == 1) % two time point calibration, run twice with same kp from the two maps
            for j = 1:2
                ...
                N_aug(:,:,t_idx) = RXDIF_2D_wMC_wAC(initial, kp, d, alpha1, alpha2, tx_params, t_out, h, dt, bcs, M, E, nu, matX, matY);
            end
