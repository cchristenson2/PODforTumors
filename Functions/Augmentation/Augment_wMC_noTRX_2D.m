function [N_aug,kp] = Augment_wMC_noTRX_2D(N, t, h, dt, bcs, bounds, ntp_cal, M, E, nu, matX, matY)
    thresh = 0.15;
    ...
    %% Run forward eval
        if(ntp_cal == 1) % two time point calibration, run twice with same kp from the two maps
            for j = 1:2
                ...
                N_aug(:,:,t_idx) = RXDIF_2D_wMC(initial, kp, d, t_out, h, dt, bcs, M, E, nu, matX, matY);
            end
