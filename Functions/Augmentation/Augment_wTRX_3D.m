function [N_aug,kp] = Augment_wTRX_3D(N, t, h, dz, dt, bcs, bounds, ntp_cal, tx_params)
    thresh = 0.15;
    ...
    %% Run forward eval
        if(ntp_cal == 1) % two time point calibration, run twice with same kp from the two maps
            for j = 1:2
                ...
                N_aug(:,:,:,t_idx) = RXDIF_3D_wAC(initial, kp, d, alpha1, alpha2, tx_params, t_out, h, dz, dt, bcs);
            end
