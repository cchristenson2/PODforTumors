...
function [params, stats, outputs, fig] = FOM_LocalCalibration_2D(tumor, ntp_cal, ntp_pred, bounds, dt, model)
...
%     [N_aug,kp_aug] = Augment_wTRX_2D(cat(3,N0,N_true), tumor.t_scan(2:end), h, 1, bcs, bounds, ntp_cal, tx_params);
    ...
    if(model==0)
        ...
        num_p  = num_kp + 1;
    elseif(model==1)
        [N_sim, TC] = RXDIF_2D_wAC_comb(N0, kp_g, d_g, alpha1_g, tx_params, t, h, dt, bcs);
        num_p  = num_kp + 2;
    else
    ...
    while(iteration<=max_it && SSE>e_tol)
        ...
        if(j_change==j_freq)
            ...
            parfor(i = 1:num_kp)
                ...
                if(model==0)
                    N_kp = RXDIF_2D(N0, kp_p, d_g, t, h, dt, bcs);
                elseif(model==1)
                    N_kp = RXDIF_2D_wAC_comb(N0, kp_p, d_g, alpha1_g, tx_params, t, h, dt, bcs);
                else
                    N_kp = RXDIF_2D_wAC(N0, kp_p, d_g, alpha1_g, alpha2_g, tx_params, t, h, dt, bcs);
                end
            ...
            if(model==0)
                N_d     = RXDIF_2D(N0, kp_g, d_p, t, h, dt, bcs);
                
                J(:,num_kp+1) = reshape(N_d - N_sim, [], 1)/dif_d;
                
            elseif(model==1)
                N_d     = RXDIF_2D_wAC_comb(N0, kp_g, d_p, alpha1_g, tx_params, t, h, dt, bcs);
                
                alpha1_p = alpha1_g*delta;
                dif_alpha1 = alpha1_p - alpha1_g;
                N_alpha1 = RXDIF_2D_wAC_comb(N0, kp_g, d_g, alpha1_p, tx_params, t, h, dt, bcs);
                
                J(:,num_kp+1) = reshape(N_d - N_sim, [], 1)/dif_d;
                J(:,num_kp+2) = reshape(N_alpha1 - N_sim, [], 1)/dif_alpha1;
                
            else
                N_d     = RXDIF_2D_wAC(N0, kp_g, d_p, alpha1_g, alpha2_g, tx_params, t, h, dt, bcs);
        ...
        elseif(model==1)
            [N_test, TC_test] = RXDIF_2D_wAC_comb(N0, kp_test, d_test, alpha1_test, tx_params, t, h, dt, bcs);
        else
    ...
    if(ntp_pred ~= 0)
        ...
        elseif(model==1)
            N_pred = RXDIF_2D_wAC_comb(N0, kp_g, d_g, alpha1_g, tx_params, tumor.t_scan(end), h, dt, bcs);
        else
