...
%     kp_g(~ROI) = 0;
    ...
    Ar_lib = buildLocalDiffuseLibrary(N_true(:,:,1), V, k, 20, dMode_bounds, tumor.h, tumor.dz, tumor.bcs);
    ...
    while(SSE>e_tol && iteration <= max_it)
        ...
        elseif(model==1)
            ...
            [N_test, TC_test] = OperatorRXDIF_2D_wMC_wAC(N0_r, d_test, H_test, Ta_test, Tc_test, tx_params, t, dt, M_r, E, nu, matX, matY, matX_r, matY_r, V, V_s, Ar_lib, k, 1);
        end
