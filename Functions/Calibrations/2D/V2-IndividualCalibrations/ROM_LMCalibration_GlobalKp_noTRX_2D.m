...
function [params, stats, outputs] = ROM_LMCalibration_GlobalKp_noTRX_2D(tumor, ntp_cal, ntp_pred, bounds, k, A_lib, B_lib, H_lib, T_lib, dt)
...
%         alpha1_test = alpha1_g; alpha2_test = alpha2_g;
    ...
    if(ntp_pred ~= 0)
        ...
        N_pred_full = RXDIF_2D(N0, kp_g*ones(size(N0)), d_g, tumor.t_scan(end), h, dt, bcs);
