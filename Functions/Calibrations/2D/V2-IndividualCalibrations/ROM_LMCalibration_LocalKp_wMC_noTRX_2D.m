...
function [params, stats, outputs, fig, temp] = ROM_LMCalibration_LocalKp_wMC_noTRX_2D(tumor, ntp_cal, ntp_pred, bounds, k, A_lib, B_lib, H_lib, T_lib, dt)
%     addpath(genpath('C:/ROM/'))
    ...
    matXY = [matX, zeros(size(matX)); zeros(size(matY)), matY];
...
%     V = getProjectionMatrix(cat(3,N0,N_true), k);
    ...
    V_s = [Vx, zeros(size(Vy)); zeros(size(Vx)), Vy];
...
%     H_f = assembleH(numel(N0), kp_g(:));
    ...
    %Initialize SSE
    [N_g_r, TC] = OperatorRXDIF_2D_wMC(N0_r, A_g, d_g, B_g, H_g, t, dt, M_r, E, nu, matX, matY, matX_r, matY_r, matXY, V, V_s, 1, bcs, tumor.h, tumor.dz);
...
%             start = tic;
            parfor(i = 1:num_kp)
                ...
                N_kp = OperatorRXDIF_2D_wMC(N0_r, A_g, d_g, B_t, H_t, t, dt, M_r, E, nu, matX, matY, matX_r, matY_r, matXY, V, V_s, 1, bcs, tumor.h, tumor.dz);
...
%             end
            ...
            N_d = OperatorRXDIF_2D_wMC(N0_r, A_t, d_p, B_g, H_g, t, dt, M_r, E, nu, matX, matY, matX_r, matY_r, matXY, V, V_s, 1, bcs, tumor.h, tumor.dz);
...
%         H_test = V'*H_f*kron(V,V);
        ...
        [N_test, TC_test] = OperatorRXDIF_2D_wMC(N0_r, A_test, d_test, B_test, H_test, t, dt, M_r, E, nu, matX, matY, matX_r, matY_r, matXY, V, V_s, 1, bcs, tumor.h, tumor.dz);       
...
%         [Ta_r,~] = OperatorInterp_T(alpha1_g, alpha2_g, bounds, Tr_lib);
        ...
        N_pred_r = OperatorRXDIF_2D_wMC(N0_r, A_r, d_g, B_r, H_r, tumor.t_scan(end), dt, M_r, E, nu, matX, matY, matX_r, matY_r, matXY, V, V_s, 1, bcs, tumor.h, tumor.dz);
