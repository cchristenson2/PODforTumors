...
function [params, stats, outputs, fig, temp] = ROM_LMCalibration_LocalKp_noTRX_3D(tumor, ntp_cal, ntp_pred, bounds, k, A_lib, B_lib, H_lib, T_lib, dt, workers)
%     addpath(genpath('C:/ROM/'))
    ...
    [N_aug, kp_aug] = Augment_noTRX_3D(cat(4,N0,N_true), tumor.t_scan(2:end), h, dz, dt, bcs, bounds, ntp_cal);
...
%         [Ta_r,~] = OperatorInterp_T(alpha1_g, alpha2_g, bounds, Tr_lib);
        ...
        N_pred_full = RXDIF_3D(N0, kp_g, d_g, tumor.t_scan(end), h, dz, dt, bcs);
