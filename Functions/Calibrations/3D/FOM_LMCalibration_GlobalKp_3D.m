...
function [params, stats, outputs] = FOM_LMCalibration_GlobalKp_3D(tumor, ntp_cal, ntp_pred, bounds, dt)
...
%     alpha2_g   = exp(log(alpha_low) + (log(alpha_up)-log(alpha_low)) * rand(1,1));
    ...
    %Initialize SSE
    [N_sim, TC] = RXDIF_3D_wAC(N0, kp_g, d_g, alpha1_g, alpha2_g, tx_params, t, h, dt, bcs);
    ...
    while(iteration<=max_it && SSE>e_tol)
        ...
        if(j_change==j_freq)
            ...
            N_kpp    = RXDIF_3D_wAC(N0, kp_p, d_g, alpha1_g, alpha2_g, tx_params, t, h, dt, bcs);
            N_dp     = RXDIF_3D_wAC(N0, kp_g, d_p, alpha1_g, alpha2_g, tx_params, t, h, dt, bcs);
            N_alpha1 = RXDIF_3D_wAC(N0, kp_g, d_g, alpha1_p, alpha2_g, tx_params, t, h, dt, bcs);
            N_alpha2 = RXDIF_3D_wAC(N0, kp_g, d_g, alpha1_g, alpha2_p, tx_params, t, h, dt, bcs);
        ...
        [N_test, TC_test] = RXDIF_3D_wAC(N0, kp_test, d_test, alpha1_test, alpha2_test, tx_params, t, h, dt, bcs);
    ...
    if(ntp_pred ~= 0)
        N_pred = RXDIF_3D_wAC(N0, kp_g, d_g, alpha1_g, alpha2_g, tx_params, tumor.t_scan(end), h, dt, bcs);
        outputs.Pred = N_pred;
    end
