...
function [params, stats, outputs] = ROM_LMCalibration_GlobalKp_3D(tumor, ntp_cal, ntp_pred, bounds, k, A_lib, B_lib, H_lib, T_lib, dt)
...
%     alpha2_g   = exp(log(alpha_low) + (log(alpha_up)-log(alpha_low)) * rand(1,1));
    ...
    if(ntp_pred ~= 0)
        ...
        N_pred_full = RXDIF_3D_wAC(N0, kp_g*ones(size(N0)), d_g, alpha1_g, alpha2_g, tx_params, tumor.t_scan(end), h, dt, bcs);
