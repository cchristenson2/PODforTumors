...
function [params, stats, outputs, fig] = FOM_LMCalibration_LocalKp_comb_3D(tumor, ntp_cal, ntp_pred, bounds, dt, workers)
...
%             start = tic;
            ...
            parfor(i = 1:num_kp)
                ...
                N_kp    = RXDIF_3D_wAC_comb(N0, kp_p, d_g, alpha1_g, tx_params, t, h, dz, dt, bcs);
...
%         end
        ...
        [N_test, TC_test] = RXDIF_3D_wAC_comb(N0, kp_test, d_test, alpha1_test, tx_params, t, h, dz, dt, bcs);
...
%         alpha2_store = [alpha2_store, alpha2_g];
    ...
    if(ntp_pred ~= 0)
        N_pred = RXDIF_3D_wAC_comb(N0, kp_g, d_g, alpha1_g, tx_params, tumor.t_scan(end), h, dz, dt, bcs);
        outputs.Pred = N_pred;
    end
