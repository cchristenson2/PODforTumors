...
function [params, stats, outputs, fig, temp] = ROM_LMCalibration_LocalKp_3D(tumor, ntp_cal, ntp_pred, bounds, k, A_lib, B_lib, H_lib, T_lib, dt, workers)
%     addpath(genpath('C:/ROM/'))
    ...
    [N_aug] = Augment_wTRX_3D(cat(4,N0,N_true), tumor.t_scan(2:end), h, dz, 1, bcs, bounds, ntp_cal, tx_params);
...
%         H_test = V'*H_f*kron(V,V);
    ...
    if(ntp_pred ~= 0)
        ...
        N_pred_full = RXDIF_3D_wAC(N0, kp_g, d_g, alpha1_g, alpha2_g, tx_params, tumor.t_scan(end), h, dz, dt, bcs);
