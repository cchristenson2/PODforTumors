...
function [params, stats, outputs] = FOM_LMCalibration_GlobalKp_noTRX_3D(tumor, ntp_cal, ntp_pred, bounds, dt)
...
%     disp('Data loaded...');
    ...
    while(iteration<=max_it && SSE>e_tol)
        ...
        if(j_change==j_freq)
            ...
            N_kpp    = RXDIF_3D(N0, kp_p, d_g, t, h, dt, bcs);
            N_dp     = RXDIF_3D(N0, kp_g, d_p, t, h, dt, bcs);
