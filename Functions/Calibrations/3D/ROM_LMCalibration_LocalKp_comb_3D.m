...
function [params, stats, outputs, fig, temp] = ROM_LMCalibration_LocalKp_comb_3D(tumor, ntp_cal, ntp_pred, bounds, k, A_lib, B_lib, H_lib, T_lib, dt, workers)
%     addpath(genpath('C:/ROM/'))
    ...
    [N_aug, kp_aug] = Augment_wTRX_comb_3D(cat(4,N0,N_true), tumor.t_scan(2:end), h, dz, dt, bcs, bounds, ntp_cal, tx_params);
...
%         alpha2_store = [alpha2_store, alpha2_g];
    ...
    if(ntp_pred ~= 0)
        ...
        N_pred_full = RXDIF_3D_wAC_comb(N0, kp_g, d_g, alpha1_g, tx_params, tumor.t_scan(end), h, dz, dt, bcs);
