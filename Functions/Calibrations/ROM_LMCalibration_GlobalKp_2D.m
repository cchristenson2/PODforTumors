...
function [params, stats, outputs] = ROM_LMCalibration_GlobalKp_2D(tumor, ntp_cal, ntp_pred, bounds, k, A_lib, B_lib, H_lib, T_lib, dt)
...
%     idx_ROI = find(ROI);
    ...
    [A_g,B_g,H_g,Ta_g,Tc_g] = OperatorInterp_wAC(kp_g,d_g,alpha1_g,alpha2_g,bounds,Ar_lib,Br_lib,Hr_lib,Tr_lib);
    ...
    while(SSE>e_tol && iteration <= max_it)
        if(j_change == j_freq)
            ...
            J = zeros(numel(N0_r)*ntp_cal, 4);

            %Perturb k
            kp_p = kp_g.*delta;
            dif = kp_p - kp_g;
            [A_t, B_t, H_t, Ta_t, Tc_t] = OperatorInterp_wAC(kp_p,d_g,alpha1_g,alpha2_g,bounds,Ar_lib,Br_lib,Hr_lib,Tr_lib);
            N_kpp = OperatorRXDIF_2D_wAC(N0_r, A_t, B_t, H_t, Ta_t, Tc_t, tx_params, t, dt);
            J(:,1) = reshape(N_kpp - N_g_r, [], 1)./dif;

            %Perturb d
            d_p = d_g.*delta;
            dif = d_p - d_g;
            [A_t, B_t, H_t, Ta_t, Tc_t] = OperatorInterp_wAC(kp_g,d_p,alpha1_g,alpha2_g,bounds,Ar_lib,Br_lib,Hr_lib,Tr_lib);
            N_dp = OperatorRXDIF_2D_wAC(N0_r, A_t, B_t, H_t, Ta_t, Tc_t, tx_params, t, dt);
            J(:,2) = reshape(N_dp - N_g_r, [], 1)./dif;

            %Perturb alpha1
            alpha1_p = alpha1_g.*delta;
            dif = alpha1_p - alpha1_g;
            [A_t, B_t, H_t, Ta_t, Tc_t] = OperatorInterp_wAC(kp_g,d_g,alpha1_p,alpha2_g,bounds,Ar_lib,Br_lib,Hr_lib,Tr_lib);
            N_alpha1 = OperatorRXDIF_2D_wAC(N0_r, A_t, B_t, H_t, Ta_t, Tc_t, tx_params, t, dt);
            J(:,3) = reshape(N_alpha1 - N_g_r, [], 1)./dif;

            %Perturb alpha2
            alpha2_p = alpha2_g.*delta;
            dif = alpha2_p - alpha2_g;
            [A_t, B_t, H_t, Ta_t, Tc_t] = OperatorInterp_wAC(kp_g,d_g,alpha1_g,alpha2_p,bounds,Ar_lib,Br_lib,Hr_lib,Tr_lib);
            N_alpha2 = OperatorRXDIF_2D_wAC(N0_r, A_t, B_t, H_t, Ta_t, Tc_t, tx_params, t, dt);
            J(:,4) = reshape(N_alpha2 - N_g_r, [], 1)./dif;
        ...
        %Run forward_check
        [A_test, B_test, H_test, Ta_test, Tc_test] = OperatorInterp_wAC(kp_test,d_test,alpha1_test,alpha2_test,bounds,Ar_lib,Br_lib,Hr_lib,Tr_lib);
    ...
    if(ntp_pred ~= 0)
        ...
        [A_r, B_r, H_r, Ta_r, Tc_r] = OperatorInterp_wAC(kp_g,d_g,alpha1_g,alpha2_g,bounds,Ar_lib,Br_lib,Hr_lib,Tr_lib);
        ...
        N_pred_full = RXDIF_2D_wAC(N0, kp_g*ones(size(N0)), d_g, alpha1_g, alpha2_g, tx_params, tumor.t_scan(end), dt, bcs);
