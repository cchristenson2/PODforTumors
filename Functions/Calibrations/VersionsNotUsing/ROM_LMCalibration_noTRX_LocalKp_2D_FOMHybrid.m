...
function [params, stats, outputs, fig, temp] = ROM_LMCalibration_noTRX_LocalKp_2D_v2(tumor, ntp_cal, ntp_pred, bounds, k, A_lib, B_lib, H_lib, T_lib, dt)
%     addpath(genpath('C:/ROM/'))
    ...
    delta_kp = 1.01;
    pass     = 11;       %Lambda reduction factor for successful updates
    fail     = 7;       %Lambda increase factor for unsuccessful updates
...
%     alpha2_g   = exp(log(alpha_low) + (log(alpha_up)-log(alpha_low)) * rand(1,1));
    ...
    N_true_vert = zeros(numel(N0), ntp_cal);
    for i = 1:ntp_cal
        N_true_r(:,i) = V'*reshape(N_true(:,:,i),[],1);
        N_true_vert(:,i) = reshape(N_true(:,:,i),[],1);
    end
...
%             J = zeros(numel(N0_r)*ntp_cal, num_p);
            J = zeros(numel(N0)*ntp_cal, num_p);
...
%                 end
                ...
                N_kp_f = zeros(numel(N0),ntp_cal);
                for j = 1:ntp_cal
                    N_kp_f(:,j) = V*N_kp(:,j);
                end
                
%                 J(:,i) = reshape(N_kp - N_g_r, [], 1)./dif_kp;
                J(:,i) = reshape(N_kp_f - N_g, [], 1)./dif_kp;
            end
%             stop = toc(start);
%             if(iteration==1)
...
%             end
            ...
            N_d_f = zeros(numel(N0),ntp_cal);
            for i = 1:ntp_cal
                N_d_f(:,i) = V*N_d(:,i);
            end
            
%             J(:,num_kp+1) = reshape(N_d - N_g_r, [], 1)./dif;
            J(:,num_kp+1) = reshape(N_d_f - N_g, [], 1)./dif;
            
            j_change = 0;
        end
        %Residuals & LM Calc
%         residuals = reshape(N_true_r - N_g_r, [], 1);
        residuals = reshape(N_true_vert - N_g, [], 1);
...
%         H_test = V'*H_f*kron(V,V);
        ...
        N_test_f = zeros(numel(N0),ntp_cal);
        for i = 1:ntp_cal
            N_test_f(:,i) = V*N_test(:,i);
        end
    ...
    if(ntp_pred ~= 0)
        ...
        N_pred_full = RXDIF_2D(N0, kp_g, d_g, tumor.t_scan(end), dt, bcs);
