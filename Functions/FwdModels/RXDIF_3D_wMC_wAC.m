...
function [N_sim, TC] = RXDIF_3D_wMC_wAC(initial, kp, d0, alpha1, alpha2, tx_params, t, h, dz, dt, bcs, M, E, nu, matX, matY, matZ)
        ...
        for z = 1:sz
            for y = 1:sy
                for x = 1:sx
                    ...
                    if(trx_on~=0)
                        treat = (alpha1*exp(-tx_params.beta1*t) + alpha2*exp(-tx_params.beta2*t))*tx_params.C(y,x,z)*N(y,x,z);
                    else
                        treat = 0;
                    end
