...
function [N_sim, TC] = OperatorRXDIF_3D_wMC_wAC_comb(N0, d, B, H, T1, tx_params, t, dt, M, E, nu, matX, matY, matZ, matX_r, matY_r, matZ_r, V, Vs, Vd, Ar_lib, k, reduced)
    ...
    for l = 2:nt
        ...
        if(trx_on~=0)
            treat = T1.*(exp(-b1*t) + exp(-b2*t))*(N(:,l-1));
        else
            treat = 0;
        end
        ...
        if(reduced==0)
            ...
            Z_dot = (matZ * N(:,l-1)) .* (matZ * (d.*S));
            N(:,l) = N(:,l-1) + dt*(S.*(A*N(:,l-1)) + (X_dot + Y_dot + Z_dot) + B*N(:,l-1) - H*kron(N(:,l-1), N(:,l-1)) - treat);
        else
            X_dot = (matX_r * N(:,l-1)) .* (matX_r * (V'*(d.*S)));
            Y_dot = (matY_r * N(:,l-1)) .* (matY_r * (V'*(d.*S)));
            Z_dot = (matZ_r * N(:,l-1)) .* (matZ_r * (V'*(d.*S)));
            N(:,l) = N(:,l-1) + dt*(A*N(:,l-1) + (X_dot + Y_dot + Z_dot) + B*N(:,l-1) - H*kron(N(:,l-1), N(:,l-1)) - treat);
        end
