...
function [tumor] = loadPatientData_fullRes_cropped(location)
    data = load(location);
    
    %Stack cell counts by time point in 4th dimension
    names = fields(data.full_res_dat_cropped);
    idx = contains(names, 'NTC');
    names(~idx) = [];
    names = sort(names);
    N = zeros([size(data.full_res_dat.NTC1), numel(names)]);
    for i = 1:numel(names)
        eval(['N(:,:,:,i) = data.full_res_dat_cropped.NTC',num2str(i),';']);
    end
    N = N./max(N(:));
    ...
    tumor.N = N;
    tumor.AUC = data.full_res_dat_cropped.AUC;
    tumor.Mask = data.full_res_dat_cropped.BreastMask;
    tumor.t_scan = t_scan;
    tumor.t_trx = t_trx;
