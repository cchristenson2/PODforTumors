...
%}
function [FOM, ROM, tumor] = calibrationComparison_global_2D(location, cal_flag, bounds, savepath)
...
%     addpath(genpath('C:/ROM/'))
    ...
    start = tic;
    [params_FOM, stats_FOM, outputs_FOM] = FOM_LMCalibration_GlobalKp_2D(tumor, ntp_cal, ntp_pred, bounds, dt);
    time_FOM = toc(start); %Seconds
    
    %Reduced order model calibration
    start = tic;
    [params_ROM, stats_ROM, outputs_ROM] = ROM_LMCalibration_GlobalKp_2D(tumor, ntp_cal, ntp_pred, bounds, 5, A_lib, B_lib, H_lib, T_lib, dt);
